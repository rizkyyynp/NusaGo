...
export default function Profile() {
    ...
    const handleUpdate = async (data) => {
        try {
            const res = await updateData('update-profile', data);
            if (res.status === 200) {
                Swal.fire({
                    icon: 'success',
                ...
                <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center justify-center">
                            <FaUserCircle className="text-3xl font-bold text-center text-primary mr-2"/>
                            <h2 className="text-2xl font-bold text-center text-primary font-podkova">Profile Information</h2>
                        </div>
                        <button
                            type="button"
                            className="text-zinc-100 bg-primary w-10 h-10 rounded-full focus:outline-none flex items-center justify-center"
            ...
            {isEditing && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-8">
                    <form onSubmit={handleSubmit} className="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center justify-center">
                                <FaEdit className="text-2xl font-bold text-center text-primary mr-2" />
                                <h2 className="text-2xl font-bold text-center text-primary font-podkova">Profile Information</h2>
                            </div>
