...
import { FaEnvelope, FaPenToSquare, FaPhone, FaUser } from 'react-icons/fa6';
...
export default function ListUser({ users, refetch }) {
    ...
    const handleUpdate = async (data) => {
        try {
            const res = await updateData(`update-user-role/${selectedUser.id}`, data);
            if (res.status === 200) {
                Swal.fire({
                    icon: 'success',
                ...
                <div className={`bg-white rounded-lg  overflow-hidden relative ${darkMode ? 'shadow-BS7' : 'shadow-BS6'}`} key={index}>
                    ...
                    <div className={`p-4 ${item.role === 'admin' ? 'bg-primary' : 'bg-dark1'} space-y-3`}>
                        ...
                        <p className="flex items-center text-zinc-100 font-nunito text-sm lg:text-md my-2">
                            <FaPhone className='mr-2 text-xl' />{item.phoneNumber}
                        </p>
                        <p className="flex items-center text-zinc-100 font-nunito text-sm lg:text-md">
                        ...
                        <div className='flex items-center justify-center mt-4'>
                            <button className='text-zinc-100 flex' onClick={() => setSelectedUser(item)}>
                                <FaPenToSquare className='text-xl pr-1' />
                                Update Role
                            </button>
                        </div>
                    </div>
